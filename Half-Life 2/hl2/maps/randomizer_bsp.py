...
def randomize_file(file_path, replacements, check_next_line=False, important_npcs=None,
                   npc_map_dict=None, npc_model_dict=None, is_weapon=False, prompt=None):
    with open(file_path, 'rb') as file:
        # Read the offset at 0x08
        file.seek(0x08, 0)
        offset_data = file.read(4)
        if len(offset_data) < 4:
            return
        start_offset = struct.unpack('<I', offset_data)[0]
        log_print(f"Reading from offset 0x{start_offset:X} in {file_path}")
    ...
    if is_weapon and os.path.basename(file_path) == "d1_trainstation_04.bsp":
        stunstick_pattern = re.compile(b'"weapon_stunstick"', re.IGNORECASE)
        for match in stunstick_pattern.finditer(content):
            start, end = match.span()
            excluded_positions.update(range(start, end))
            log_print(f"Found 'weapon_stunstick' in 'd1_trainstation_04.bsp'. Will skip randomization as it's needed for progression.")
    ...
    if replacements:
        ...
        def replacer(match):
            ...
            # Skip if before start_offset
            if start < start_offset:
                return match.group(0)

            # Check for null bytes before this position
            null_pos = content[start_offset:start].find(b'\x00')
            if null_pos != -1:
                actual_pos = start_offset + null_pos
                log_print(f"Found null byte at position 0x{actual_pos:X} in {file_path}, stopping randomization")
                return match.group(0)
            ...
            # Check for "d1_canals_01a.bsp" and "d1_canals_02.bsp"
            if os.path.basename(file_path) == "d1_canals_01a.bsp" or os.path.basename(file_path) == "d1_canals_02.bsp":
            ...
            # Check for "d2_prison_07.bsp", "d2_prison_08.bsp" and "d3_c17_11.bsp"
            if os.path.basename(file_path) == "d2_prison_07.bsp" or os.path.basename(file_path) == "d2_prison_08.bsp" or os.path.basename(file_path) == "d3_c17_11.bsp":
            ...
            # Check for "d1_trainstation_06.bsp"
            if os.path.basename(file_path) == "d1_trainstation_06.bsp" and original == '"weapon_crowbar"' and replacement in ['"weapon_bugbait"', '"weapon_pistol"', '"weapon_frag"', '"weapon_stunstick"']:
                log_print(f"Tried randomizing 'weapon_crowbar' to '{replacement}' in 'd1_trainstation_06.bsp'. Retrying with a different weapon as this weapon will cause you to not be able to continue.")
                while replacement in ['"weapon_bugbait"', '"weapon_pistol"', '"weapon_frag"', '"weapon_stunstick"']:
                    replacement = random.choice(replacements[original]).strip()
                    replacement_bytes = replacement.encode('utf-8').strip()

            # Check for "d2_coast_05.bsp"
            if os.path.basename(file_path) == "d2_coast_05.bsp" and replacement in ['"models/props_buildings/row_corner_1_fullscale.mdl"']:
                log_print(f"Tried randomizing '{original}' to '{replacement}' in 'd2_coast_05.bsp'. Retrying with a different prop as it will block your way.")
                while replacement in ['"models/props_buildings/row_corner_1_fullscale.mdl"']:
                    replacement = random.choice(replacements[original]).strip()
                    replacement_bytes = replacement.encode('utf-8').strip()

            # Check for "d2_coast_07.bsp"
            if os.path.basename(file_path) == "d2_coast_07.bsp" and replacement in ['"models/props_buildings/row_res_1_fullscale.mdl"']:
                log_print(f"Tried randomizing '{original}' to '{replacement}' in 'd2_coast_07.bsp'. Retrying with a different prop as it will block your way.")
                while replacement in ['"models/props_buildings/row_res_1_fullscale.mdl"']:
                    replacement = random.choice(replacements[original]).strip()
                    replacement_bytes = replacement.encode('utf-8').strip()

            # Check for "d3_breen_01.bsp"
            if os.path.basename(file_path).lower() == "d3_breen_01.bsp" and original == '"weapon_physcannon"':
            ...
            # Check for "d1_trainstation_01.bsp", "d3_c17_10a.bsp" and "d1_trainstation_03.bsp"
            if os.path.basename(file_path).lower() == "d1_trainstation_01.bsp" or os.path.basename(file_path).lower() == "d3_c17_10a.bsp" or os.path.basename(file_path).lower() == "d1_trainstation_03.bsp":
                ...
                while os.path.basename(file_path).lower() == "d1_trainstation_01.bsp" or os.path.basename(file_path).lower() == "d3_c17_10a.bsp" or os.path.basename(file_path).lower() == "d1_trainstation_03.bsp":
                    return match.group(0)
                
            # Check for "d1_trainstation_04.bsp"
            if os.path.basename(file_path).lower() == "d1_trainstation_04.bsp" and original in ['"models/props_lab/powerbox02b.mdl"', '"models/props_lab/pipesystem01a.mdl"', '"models/props_lab/pipesystem02a.mdl"', '"models/props_c17/door01_left.mdl"', '"models/props_lab/keypad.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_trainstation_04.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d1_trainstation_05.bsp"
            if os.path.basename(file_path).lower() == "d1_trainstation_05.bsp" and original in ['"models/props_interiors/vendingmachinesoda01a_door.mdl"', '"models/props_c17/door01_left.mdl"', '"models/props_lab/keypad.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_trainstation_05.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Remove wood pallets from "d1_canals_01.bsp"
            if os.path.basename(file_path).lower() == "d1_canals_01.bsp" and original in ['"models/props_junk/wood_pallet001a.mdl"']:
            ...
            # Check for "d1_canals_01a.bsp"
            if os.path.basename(file_path).lower() == "d1_canals_01a.bsp" and original in ['"models/props_c17/oildrum001_explosive.mdl"', '"models/props_canal/canal_bars002.mdl"', '"models/props_canal/canal_bars002b.mdl"', '"models/props_junk/cinderblock01a.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_canals_01a.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d1_canals_02.bsp"
            if os.path.basename(file_path).lower() == "d1_canals_02.bsp" and original in ['"models/props_junk/cinderblock01a.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_canals_02.bsp' because randomizing these props cause potential progression issues.")
                while original in ['"models/props_junk/cinderblock01a.mdl"']:
                    return match.group(0)
                
            # Check for "d1_canals_06.bsp"
            if os.path.basename(file_path).lower() == "d1_canals_06.bsp" and original in ['"models/props_borealis/bluebarrel002.mdl"', '"models/props_canal/canal_bars003.mdl"', '"models/props_c17/oildrum001.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_canals_06.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d1_canals_12.bsp"
            if os.path.basename(file_path).lower() == "d1_canals_12.bsp" and original in ['"models/props_c17/oildrum001_explosive.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_canals_12.bsp' because randomizing these props cause potential progression issues.")
                while original in ['"models/props_c17/oildrum001_explosive.mdl"']:
                    return match.group(0)

            # Check for "d1_eli_01.bsp"
            if os.path.basename(file_path).lower() == "d1_eli_01.bsp" and original in ['"models/props_lab/blastdoor001a.mdl"', '"models/props_lab/blastdoor001b.mdl"', '"models/props_lab/blastdoor001c.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_eli_01.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d1_eli_02.bsp"
            if os.path.basename(file_path).lower() == "d1_eli_02.bsp" and original in ['"models/props_lab/dogobject_wood_crate001a_damagedmax.mdl"', '"models/props_c17/oildrum001.mdl"', '"models/props_lab/blastdoor001a.mdl"', '"models/props_lab/blastdoor001b.mdl"', '"models/props_lab/blastdoor001c.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_eli_02.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d1_town_01.bsp"
            if os.path.basename(file_path).lower() == "d1_town_01.bsp" and original in ['"models/props_vehicles/car004a_physics.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_town_01.bsp' because randomizing these props cause potential progression issues.")
                while original in ['"models/props_vehicles/car004a_physics.mdl"']:
                    return match.group(0)
                
            # Check for "d1_town_02.bsp"
            if os.path.basename(file_path).lower() == "d1_town_02.bsp" and original in ['"models/props_wasteland/tram_lever01.mdl"', '"models/props_c17/pulleywheels_small01.mdl"', '"models/props_c17/pulleywheels_large01.mdl"', '"models/props_wasteland/tram_bracket01.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd1_town_02.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d2_coast_03.bsp"
            if os.path.basename(file_path).lower() == "d2_coast_03.bsp" and original in ['"models/props_wasteland/barricade002a.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd2_coast_03.bsp' to prevent crashing issue.")
                while original in ['"models/props_wasteland/barricade002a.mdl"']:
                    return match.group(0)

            # Check for "d2_coast_11.bsp"
            if os.path.basename(file_path).lower() == "d2_coast_11.bsp" and original in ['"models/props_borealis/borealis_door001a.mdl"', '"models/props_wasteland/exterior_fence003b.mdl"', '"models/props_foliage/driftwood_01a.mdl"', '"models/props_debris/rebar_cluster001b.mdl"', '"models/props_junk/wood_pallet001a.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd2_coast_11.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d2_prison_05.bsp"
            if os.path.basename(file_path).lower() == "d2_prison_05.bsp" and original in ['"models/props_c17/oildrum001.mdl"', '"models/props_junk/wood_crate001a_damaged.mdl"', '"models/props_junk/cardboard_box002a.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd2_prison_05.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d2_prison_07.bsp"
            if os.path.basename(file_path).lower() == "d2_prison_07.bsp" and original in ['"models/props_borealis/bluebarrel001.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd2_prison_07.bsp' because randomizing these props cause potential progression issues.")
                while original in ['"models/props_borealis/bluebarrel001.mdl"']:
                    return match.group(0)
                
            # Check for "d3_c17_08.bsp"
            if os.path.basename(file_path).lower() == "d3_c17_08.bsp" and original in ['"models/props_c17/oildrum001.mdl"', '"models/props_c17/oildrum001_explosive.mdl"', '"models/props_c17/handrail04_long.mdl"', '"models/props_c17/handrail04_brokenlong.mdl"', '"models/props_c17/utilityconnecter005.mdl"']:
                log_print(f"Skipping randomization for '{original}' in 'd3_c17_08.bsp' because randomizing these props cause potential progression issues.")
            ...
            # Check for "d3_breen_01.bsp"
            if os.path.basename(file_path).lower() == "d3_breen_01.bsp" and prompt == "props":
                log_print(f"Skipping prop randomization in 'd3_breen_01.bsp' because it causes progression issues.")
                return match.group(0)
                
            # Check for "d3_c17_06a.bsp"
            concrete_models = read_concrete_models()
            if os.path.basename(file_path).lower() == "d3_c17_06a.bsp" and original in concrete_models:
                log_print(f"Skipping randomization for '{original}' in 'd3_c17_06a.bsp' because randomizing these props cause potential progression issues.")
...
def calculate_length_and_update(file_path, bytes_added):
    import struct

    with open(file_path, 'r+b') as file:
        # 1) Read the original length from offset 0x0C
        file.seek(0x0C, 0)
        stored_len_data = file.read(4)
        if len(stored_len_data) < 4:
            return
        stored_len = struct.unpack('<I', stored_len_data)[0]

        # 2) Calculate the new length from the stored length plus the bytes changed
        new_len = stored_len + bytes_added

        # 3) Store the new length at offset 0x0C
        file.seek(0x0C, 0)
        file.write(struct.pack('<I', new_len))

        # 4) Calculate length difference
        length_diff = new_len - stored_len

        # 5) Offsets we need to update
        offsets_to_fix = [
            0x98, 0xF8, 0x1C8, 0x1D8, 0x1E8, 0x1F8, 0x238, 0x248, 0x288,
            0x298, 0x2A8, 0x2B8, 0x2C8, 0x2D8, 0x2E8, 0x328, 0x368, 0x3B8, 0x3C8
        ]

        # Update these offsets only if they're within the new length
        for off in offsets_to_fix:
            if off + 4 <= new_len:
                file.seek(off, 0)
                data = file.read(4)
                if len(data) == 4:
        ...
        # 6) Go to offset stored at 0x238 and update more offsets if magic matches
        magic_list = [b'prps', b'prpd', b'tlpd', b'hlpd']
        
        # Read the offset value at 0x238
        file.seek(0x238, 0)
        offset_data = file.read(4)
        if len(offset_data) < 4:
            return
            
        chunk_offset = struct.unpack('<I', offset_data)[0]
        log_print(f"Read offset 0x{chunk_offset:X} from position 0x238 in {os.path.basename(file_path)}")
        
        # Check magic and update offsets at +0x0C if valid
        for i in range(4):
            magic_off = chunk_offset + 0x04 + (i * 0x10)
            offset_off = chunk_offset + 0x0C + (i * 0x10)
            
            file.seek(magic_off, 0)
            magic_data = file.read(4)
            if not magic_data or len(magic_data) < 4:
                log_print(f"Reached actual EOF at offset 0x{magic_off:X}, stopping magic check")
                break

            if magic_data in magic_list:
                log_print(f"Found magic '{magic_data.decode('ascii')}' at offset 0x{magic_off:X}")
                file.seek(offset_off, 0)
                off_data = file.read(4)
                if len(off_data) == 4:
                    val = struct.unpack('<I', off_data)[0]
                    old_val = val
                    val += length_diff
                    log_print(f"Updating offset at 0x{offset_off:X} from {old_val} to {val}")
                    file.seek(offset_off, 0)
                    file.write(struct.pack('<I', val))
            else:
                log_print(f"No valid magic found at 0x{magic_off:X} (found '{magic_data}'), stopping check")
                break
...
def remove_coast_11_blockers(file_path):
    if os.path.basename(file_path).lower() != "d2_coast_11.bsp":
        return
    ...
    # These are the props/entities to remove from d2_coast_11.bsp
    entity_names = [
